...
for(STATE in state_list){
  ...
  deathPreds.array.weekly <- apply(deathPreds.array.weekly.temp,c(1,3),function(x) sum(x*weights))
  deathPredsCum.array.weekly.temp = apply(deathPredsCum.array,c(1,3),function(x)
    x[forecast.row+7*(1:6)-(forecast.day+1)%%7])
  deathPredsCum.array.weekly <- apply(deathPredsCum.array.weekly.temp,c(1,3),function(x) sum(x*weights))
