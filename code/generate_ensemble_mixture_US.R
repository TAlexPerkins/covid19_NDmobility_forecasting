...
for(STATE in state_list){
  ...
  for(ii in 1:length(f)){
    ...
    choose.this <- matrix(rep(ifelse(sample.models==ii,TRUE,FALSE),dim(deathPreds)[1]),
                          dim(deathPreds)[1], byrow=TRUE)
    if (nrow(deathPreds)==nrow(deathPreds.array)){
        deathPreds.array = 
            deathPreds.array + ifelse(!choose.this | is.na(deathPreds),
                                      0,deathPreds)
    } else {
        row.n <- nrow(deathPreds.array)
        deathPreds.array = 
            deathPreds.array + ifelse(!choose.this | is.na(deathPreds[1:row.n,]),
                                      0,deathPreds[1:row.n,])
    }
