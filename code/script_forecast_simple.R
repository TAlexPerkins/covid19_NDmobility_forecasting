...
## check if model will be used in ensemble
load(paste0("../output/weights_", STATE, ".RData"))
training.files <- list.files("../output/",paste0("training_",STATE))
trained.models <- as.numeric(str_extract(training.files, "\\-*\\d+\\.*\\d*"))
run.this.model <- ifelse(MOBILITY %in% trained.models[weights > 0.0], TRUE, FALSE)
